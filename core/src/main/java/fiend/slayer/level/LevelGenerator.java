...
public class LevelGenerator {
    ...
    public TiledMap generateLevel() {
        ...
        // mob spawn
        MapLayer mob_spawn_layer = new MapLayer(); mob_spawn_layer.setName("mob_spawn"); layers.add(mob_spawn_layer);
        MapObjects mobmapobjs = mob_spawn_layer.getObjects();

        for (Room room : rooms) {
            room.genRoom(lay, cell, colmapobjs, mobmapobjs);
        }
        ...
        player_spawn.getRectangle().x = lowx + MAX_ROOM_SIDELEN/2; player_spawn.getRectangle().y = lowy + MAX_ROOM_SIDELEN/2;
